...
process panel_var_annotate {
  tag "${run_id}"
  container 'ghcr.io/mvz-hp/panel_var_annotate:1.0.7'
  ...
  input:
    tuple path(snv_dir), val(run_id), val(publish_dir)
  ...
  script:
    ...
    if (params.mode == 'wes') {
      ...
      mincov = params.mincov ?: 5000
      minvaf = params.minvaf ?: 0.1
      minvad = params.minvad ?: 3
    }
